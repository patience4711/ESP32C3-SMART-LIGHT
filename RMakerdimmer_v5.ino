...
void setup() {
  ...
  my_device = new Device("Dimmer", ESP_RMAKER_DEVICE_LIGHTBULB, &gpio_dimmer);
  if (!my_device) {
    return;
  }
  
  my_device->addNameParam("Dimmer");
  ...
  ledblink(3, 300); // this provides some time for the server to get in the air
    ...
    //if(counTer < 30) Serial.println("wifi connected");
    Serial.println("\npauze 1 . . . . . . . . . . ");
    delay(5000);
    RMaker.start();
    Serial.println("\npauze 2 . . . . . . . . . . ");
   ...
   // we delay the rainmaker start to get rid of connection problems
   //RMaker.start();
